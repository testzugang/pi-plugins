...
**Architecture:** A modular Pi extension structure consisting of standalone, readable TypeScript files (`settings.ts`, `utils.ts`, `breadcrumb.ts`, `editor.ts`, `header.ts`, `footer.ts`, `index.ts`) communicating via local APIs. It manages and hooks Pi events (`session_start`, `message_update`, etc.) to update status bar lines and editor widgets in real-time. It extends the native Pi `CustomEditor` class and registers it to draw breadcrumbs (Model ➜ Folder) inside the editor's top border.
...
### Task 3: Breadcrumb Calculation and CustomEditor Patching (`breadcrumb.ts`, `editor.ts`)
    ...
    if (breadcrumbMode === "inner" && liveCtx && currentTheme) {
      ...
      if (paddingLen >= 0) {
        ...
        result[0] =
          ...
          displayInfo +
          " " +
          currentTheme.fg("borderAccent", "─".repeat(paddingLen));
      }
    }
...
git commit -m "feat(pi-tui-hud): implement CustomEditor override with integrated breadcrumb rendering"
```
        ...
        render(width: number): string[] {
          ...
          if (s["header-info"]) {
            infoLines.push(
              theme.fg(
                "dim",
                ` Model: ${ctx.model?.id || "unknown"} | CWD: ${ctx.cwd}`,
              ),
            );
          }
          return infoLines;
...
### Task 5: Core Status-Bar & Token/Cost Tracker Footer (`footer.ts`)
      ...
      return {
        render(width: number): string[] {
          ...
          const gitSegment = branch
            ? theme.fg("success", withIcon("⎇", branch))
            : "";
          ...
          const tokens =
            contextUsage?.tokens ?? totalInput + totalOutput + totalCacheRead;
          const maxWindow =
          ...
          const extensionStatuses = footerData.getExtensionStatuses() as Map<
            string,
            string
          >;
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerCommand("hud", {
    description:
      "Configure HUD layout and features. Usage: /hud <info|breadcrumb:<mode>|footer:<on|off>|header:<on|off>>",
    handler: async (args, ctx) => {
      ...
      if (colonIdx === -1) {
        ctx.ui.notify(
          "Invalid command format. Use /hud <info|breadcrumb:top|inner|hide|footer:on|off>",
          "error",
        );
        return;
      }
