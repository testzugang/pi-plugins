...
- **Verhalten:**
  ...
  2. **Vollständige Verankerungs-Enclosure (Sicherheit & Robustheit):**
     ...
     - _Beispiel:_ Aus `r:git status|ls -la` wird intern `/^(?:git status|ls -la)$/`. Aus `r:^git status$` wird `/^(?:^git status$)$/`. Dies garantiert, dass die gesamte Befehlszeile matchen muss und verhindert Injection-Angriffe an den Rändern.
