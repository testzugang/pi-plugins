...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerCommand("hud", {
    description:
      "Configure HUD layout and features. Usage: /hud <info|breadcrumb:<hide|top|inner>|footer:<on|off>|header:<on|off>|header-info:<on|off>>",
    handler: async (args, ctx) => {
      ...
      if (!arg) {
        ...
        try {
          ...
          ctx.ui.notify(
            `Failed to save HUD settings: ${err instanceof Error ? err.message : String(err)}`,
            "error",
          );
        }
        return;
      ...
      if (colonIdx === -1) {
        ctx.ui.notify(
          "Invalid command format. Use /hud <info|breadcrumb:<hide|top|inner>|footer:<on|off>|header:<on|off>|header-info:<on|off>>",
          "error",
        );
        return;
      }
      ...
      if (key === "breadcrumb") {
        if (!["hide", "top", "inner"].includes(val)) {
          ctx.ui.notify(
            "Breadcrumb mode must be: hide, top, or inner",
            "warning",
          );
          return;
        }
