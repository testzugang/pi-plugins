...
export class HandoffOverlayComponent implements Focusable {
  ...
  private row(th: Theme, content: string): string {
    ...
    return (
      th.fg("border", "│") + this.pad(content, innerW) + th.fg("border", "│")
    );
  }
  ...
  private handleOptionsInput(data: string) {
    if (matchesKey(data, "return")) {
      if (this.selectedRow === OptionsRow.GenerateButton) {
        // "Generate" button
        const opts: HandoffOptions = {
          goal: this.goal,
          targetModel:
            this.availableModels[this.targetModelIndex] ||
        ...
        if (this.onGenerate) {
          ...
          this.onGenerate(opts, this.abortController.signal)
            ...
            .catch((err: unknown) => {
              ...
              this.ctx.ui.notify(
                `Prompt generation failed: ${errorMsg}`,
                "error",
              );
              this.phase = "options";
              this.invalidate();
      ...
      } else if (this.selectedRow === OptionsRow.CancelButton) {
        // "Cancel" button
        this.done(undefined);
      } else {
        // Move cursor to next input row
        this.selectedRow = Math.min(
          OptionsRow.CancelButton,
          this.selectedRow + 1,
        );
      }
      return;
    ...
    if (this.selectedRow === OptionsRow.Goal) {
      ...
      if (
        ...
        this.targetModelIndex =
          (this.targetModelIndex + dir + this.availableModels.length) %
  ...
  private handlePreviewInput(data: string) {
    if (this.selectedRow === PreviewRow.Editor) {
      // Text editor focus
      if (matchesKey(data, "down")) {
        if (this.previewCursorRow < this.previewLines.length - 1) {
          this.previewCursorRow++;
          this.previewCursorCol = Math.min(
            this.previewCursorCol,
            this.previewLines[this.previewCursorRow]!.length,
          );
      ...
      if (matchesKey(data, "backspace")) {
        if (this.previewCursorCol > 0) {
          this.previewLines[this.previewCursorRow] =
            currentLine.slice(0, this.previewCursorCol - 1) +
            currentLine.slice(this.previewCursorCol);
          this.previewCursorCol--;
        } else if (this.previewCursorRow > 0) {
        ...
        this.previewLines[this.previewCursorRow] =
          currentLine.slice(0, this.previewCursorCol) +
          data +
          currentLine.slice(this.previewCursorCol);
        this.previewCursorCol++;
      }
      ...
      if (matchesKey(data, "left") || matchesKey(data, "right")) {
        this.selectedRow =
          this.selectedRow === PreviewRow.ConfirmButton
            ? PreviewRow.BackButton
            : PreviewRow.ConfirmButton;
        return;
      }
      if (matchesKey(data, "return")) {
        if (this.selectedRow === PreviewRow.ConfirmButton) {
          // Confirm & Switch
          this.done({
            options: {
              goal: this.goal,
  ...
  private handleTextInput(
    field: "goal" | "sessionName" | "references",
    data: string,
  ) {
    let text =
      field === "goal"
        ...
        : field === "sessionName"
          ? this.sessionName
          : this.manualReferences;
    let cursor =
      field === "goal"
        ? this.goalCursor
        : field === "sessionName"
          ? this.sessionNameCursor
          : this.referencesCursor;
  ...
  private renderOptions(th: Theme): string[] {
    ...
    lines.push(
      this.row(
        th,
        ` ${th.bold(th.fg("accent", "🤝 Create New Handoff Session"))}`,
      ),
    );
    ...
    const renderInputField = (
      ...
      const labelStr = isSelected
        ? th.fg("accent", this.pad(`${label}:`, 18))
        : th.fg("text", this.pad(`${label}:`, 18));
      ...
      if (valueDisplay.length > 44) {
        ...
        valueDisplay =
          (start > 0 ? "..." : "") + valueDisplay.slice(start, start + 40);
      ...
      if (isSelected) {
        ...
        const cursorChar =
          renderCursor < valueDisplay.length ? valueDisplay[renderCursor] : " ";
        ...
        valueDisplay =
          before + marker + th.bold(th.fg("accent", cursorChar)) + after;
      } else {
        valueDisplay = th.fg(
          "dim",
          value.length > 44 ? value.slice(0, 41) + "..." : value || "(empty)",
        );
      }
    ...
    const rawModel =
      this.availableModels[this.targetModelIndex] || "None configured";
    const activeModel =
      rawModel.length > 44 ? rawModel.slice(0, 41) + "..." : rawModel;
    const modelPrefix =
      this.selectedRow === OptionsRow.TargetModel ? " ▶ " : "   ";
    const modelLabel =
      this.selectedRow === OptionsRow.TargetModel
        ? th.fg("accent", this.pad("Target Model:", 18))
        : th.fg("text", this.pad("Target Model:", 18));
    const modelVal =
      this.selectedRow === OptionsRow.TargetModel
        ? `◀ ${th.fg("success", activeModel)} ▶`
        : th.fg("dim", activeModel);
    ...
    lines.push(
      renderInputField(
        ...
        this.sessionName,
        this.sessionNameCursor,
        this.selectedRow === OptionsRow.SessionName,
      ),
    );
    lines.push(
      renderInputField(
        ...
        this.manualReferences,
        this.referencesCursor,
        this.selectedRow === OptionsRow.References,
      ),
    );

    const savePrefix =
      this.selectedRow === OptionsRow.SaveHandoff ? " ▶ " : "   ";
    const saveLabel =
      this.selectedRow === OptionsRow.SaveHandoff
        ? th.fg("accent", this.pad("Save Handoff File:", 18))
        : th.fg("text", this.pad("Save Handoff File:", 18));
    const toggleVal = this.saveHandoff
      ? th.fg("success", "[X] Yes")
      : th.fg("muted", "[ ] No (Draft only)");
    ...
    const btnGen =
      this.selectedRow === OptionsRow.GenerateButton
        ? th.bold(th.fg("success", "[ Generate Handoff Prompt ]"))
        : th.fg("accent", "[ Generate Handoff Prompt ]");
    const btnCancel =
      this.selectedRow === OptionsRow.CancelButton
    ...
    lines.push(
      this.row(
        th,
        ` ${th.fg("dim", "↑↓ navigate • type to edit • ←→ select option • Enter confirm • Esc exit")}`,
      ),
    );
    lines.push(th.fg("border", `╰${"─".repeat(innerW)}╯`));
  ...
  private renderLoading(th: Theme): string[] {
    ...
    lines.push(
      this.row(
        th,
        ` ${th.bold(th.fg("accent", "🤝 Generating Handoff Prompt"))}`,
      ),
    );
    lines.push(this.row(th, ""));
    lines.push(
      this.row(
        th,
        "  Please wait while the active model condenses conversation context...",
      ),
    );
    lines.push(this.row(th, ""));
  ...
  private renderPreview(th: Theme): string[] {
    ...
    lines.push(
      this.row(
        th,
        ` ${th.bold(th.fg("accent", "📝 Edit Generated Prompt Preview"))}`,
      ),
    );
    lines.push(this.row(th, ""));
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const isCurrentLine =
        isEditorSelected && lineIndex === this.previewCursorRow;
      ...
      lines.push(
        th.fg("border", "│") +
          "  " +
          displayVal +
          " ".repeat(Math.max(0, innerW - 4 - visibleWidth(lineText))) +
          "  " +
          th.fg("border", "│"),
      );
    }
    ...
    const btnBack =
      this.selectedRow === PreviewRow.BackButton
    ...
    lines.push(
      this.row(
        th,
        ` ${th.fg("dim", "↑↓ navigate/edit prompt • Enter on buttons to submit • Esc cancel")}`,
      ),
    );
    lines.push(th.fg("border", `╰${"─".repeat(innerW)}╯`));
